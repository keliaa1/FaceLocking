...
- DB is expected from enroll: data/db/face_db.npz
...
def _bbox_from_5pt(
    ...
    """
    Build a nicer face-like bbox from 5 points
...
class HaarFaceMesh5pt:
    ...
    def __init__(
        ...
        # IMPORTANT:
        # We run FaceMesh on ROI (one face per ROI),
        # so max_num_faces=1.
    ...
    def detect(
        ...
        for (x, y, w, h) in faces:
            ...
            if kps_roi is None:
                if self.debug:
                    print(
                        "[recognize] FaceMesh none "
                        "for ROI -> skip"
                    )

                continue

            # Map ROI keypoints back
...
def main():
    ...
    while True:
        ...
        x0 = w - thumb - pad
        y0 = 80  # moved down to avoid text overlay area
