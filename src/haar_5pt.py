...
class Haar5ptDetector:
    ...
    def detect(
        ...
        if kps is None:
            ...
            if self.debug:
                print(
                    ...
                    "FaceMesh returned none -> reject"
                )

            return []

        # OPTIONAL: require FaceMesh points to fall reasonably
        # inside Haar box (prevents random FaceMesh on background)
        ...
        if inside.mean() < 0.60:
            if self.debug:
                print(
                    "[haar_5pt] FaceMesh points not consistent "
                    "with Haar box -> reject"
                )
        ...
        # Haar doesn't provide a probability;
        # use a stable placeholder score
        score = 1.0
