...
def cosine_similarity(a: np.ndarray, b: np.ndarray) -> float:
    ...
    # Embeddings are already L2-normalized in embed,
...
def load_embeddings_for_person(
    ...
    for img_path in imgs:
        ...
        # If someone accidentally saved non-aligned crops,
        # skip them (keeps eval clean).
        if (
...
def sweep_thresholds(
    ...
    # FAR: impostor accepted => dist <= threshold
    # FRR: genuine rejected => dist > threshold
    results = []
...
def main():
    ...
    # Choose threshold with FAR <= target_far
    # and minimal FRR.
    best = None
